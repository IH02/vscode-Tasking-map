...
async function collectMapFilesFromFolder(folder: vscode.Uri, results: vscode.Uri[]): Promise<void> {
  ...
  for (const [name, type] of entries) {
    ...
    if (type === vscode.FileType.File && name.toLowerCase().endsWith('.map')) {
      results.push(child);
    }
  }
}
...
async function ensureMapSelected(): Promise<boolean> {
  ...
  const openMaps = vscode.workspace.textDocuments
    .filter(d => d.uri.fsPath.toLowerCase().endsWith(".map"))
    .map(d => d.uri);
...
async function findMapFileInWorkspace(): Promise<vscode.Uri | null> {
  if (mapFileUri) return mapFileUri;
  const ok = await ensureMapSelected();
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  const mapWatcher = vscode.workspace.createFileSystemWatcher('**/*.map');
  ...
  vscode.commands.registerCommand('taskingMap.goToSymbol', async (symbolName?: string) => {
    ...
    if (!mapFileUri) {
      vscode.window.showErrorMessage('No MAP file found in workspace');
      return;
    }
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('taskingMap.selectMapFile', async () => {
      ...
      const selected = await vscode.window.showQuickPick(
        ...
        { placeHolder: 'Select a MAP file to use' }
      );

      if (selected) {
        await setCurrentMapFromUri(selected.uri, selected.allowExternal);
        if (mapFileUri) {
          vscode.window.showInformationMessage(`Using MAP file: ${selected.label}`);
          updateViews();
        }
      }
    })
